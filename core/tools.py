"""
NEXUS Tool Registry & Dispatch System — Extended Developer Edition v4.0
44 built-in tools: 6 base + 10 developer + 6 advanced + 6 web + 4 vision + 7 ai_media + 5 hermes
...
AI MEDIA TOOLS (7):
  ai_image, ai_image_edit, ai_image_search, ai_vision, ai_video, ai_tts, ai_asr

HERMES-INSPIRED TOOLS (5):
  rag_ingest, rag_search, self_reflect, plan_task, self_correct
"""
...
class ToolRegistry:
    ...
    def _register_defaults(self):
        ...
        # ─── AI Media Tools (7) ───
        self.register("ai_image", "AI-Bild generieren via z-ai",
                      self._tool_ai_image,
                      {"prompt": "str", "size": "str", "output": "str"}, category="ai_media")
        self.register("ai_image_edit", "AI-Bild bearbeiten via z-ai",
                      self._tool_ai_image_edit,
                      {"prompt": "str", "image": "str", "output": "str", "size": "str"}, category="ai_media")
        self.register("ai_image_search", "Bilder suchen via z-ai",
                      self._tool_ai_image_search,
                      {"query": "str", "count": "int", "gl": "str"}, category="ai_media")
        self.register("ai_vision", "Bild-Analyse via VLM (z-ai vision)",
                      self._tool_ai_vision,
                      {"prompt": "str", "image": "str"}, category="ai_media")
        self.register("ai_video", "AI-Video generieren via z-ai",
                      self._tool_ai_video,
                      {"prompt": "str", "duration": "int", "quality": "str"}, category="ai_media")
        self.register("ai_tts", "Text-to-Speech via z-ai",
                      self._tool_ai_tts,
                      {"text": "str", "voice": "str", "speed": "float", "format": "str"}, category="ai_media")
        self.register("ai_asr", "Speech-to-Text via z-ai",
                      self._tool_ai_asr,
                      {"file": "str", "base64": "str"}, category="ai_media")

        # ─── Hermes-Inspired Tools (5) ───
        self.register("rag_ingest", "Dokument in RAG-Wissensdatenbank aufnehmen",
                      self._tool_rag_ingest,
                      {"source": "str", "type": "str"}, category="knowledge")
        self.register("rag_search", "RAG-Wissensdatenbank durchsuchen",
                      self._tool_rag_search,
                      {"query": "str", "top_k": "int"}, category="knowledge")
        self.register("self_reflect", "Eigene Ausgabequalität reflektieren",
                      self._tool_self_reflect,
                      {"task": "str", "result": "str"}, category="meta")
        self.register("plan_task", "Schrittweisen Plan erstellen",
                      self._tool_plan_task,
                      {"task": "str", "agents": "str", "tools": "str"}, category="meta")
        self.register("self_correct", "Code mit Fehlerinfo selbst korrigieren",
                      self._tool_self_correct,
                      {"task": "str", "code": "str", "error": "str"}, category="meta")
    ...
    # ═══════════════════════════════════════════════════
    # AI MEDIA TOOL IMPLEMENTATIONS (7)
    # ═══════════════════════════════════════════════════

    @staticmethod
    def _tool_ai_image(prompt: str, size: str = "1024x1024", output: str = "") -> dict:
        """AI-Bild generieren via z-ai."""
        if not ZAI_AVAILABLE:
            return {"error": "Module not available: core.zai_integration is not installed"}
        try:
            zai = get_zai()
            result = zai.generate_image(prompt=prompt, size=size, output=output)
            if isinstance(result, dict):
                return {"prompt": prompt, "size": size, "output": output, "result": str(result)[:5000]}
            return {"prompt": prompt, "size": size, "output": output, "result": str(result)[:5000]}
        except Exception as e:
            return {"error": f"ai_image fehlgeschlagen: {str(e)}"}

    @staticmethod
    def _tool_ai_image_edit(prompt: str, image: str = "", output: str = "", size: str = "1024x1024") -> dict:
        """AI-Bild bearbeiten via z-ai."""
        if not ZAI_AVAILABLE:
            return {"error": "Module not available: core.zai_integration is not installed"}
        try:
            zai = get_zai()
            result = zai.edit_image(prompt=prompt, image=image, output=output, size=size)
            if isinstance(result, dict):
                return {"prompt": prompt, "image": image, "output": output, "size": size, "result": str(result)[:5000]}
            return {"prompt": prompt, "image": image, "output": output, "size": size, "result": str(result)[:5000]}
        except Exception as e:
            return {"error": f"ai_image_edit fehlgeschlagen: {str(e)}"}

    @staticmethod
    def _tool_ai_image_search(query: str, count: int = 5, gl: str = "us") -> dict:
        """Bilder suchen via z-ai."""
        if not ZAI_AVAILABLE:
            return {"error": "Module not available: core.zai_integration is not installed"}
        try:
            zai = get_zai()
            result = zai.search_images(query=query, count=count, gl=gl)
            if isinstance(result, dict):
                return {"query": query, "count": count, "gl": gl, "result": str(result)[:5000]}
            return {"query": query, "count": count, "gl": gl, "result": str(result)[:5000]}
        except Exception as e:
            return {"error": f"ai_image_search fehlgeschlagen: {str(e)}"}

    @staticmethod
    def _tool_ai_vision(prompt: str, image: str = "") -> dict:
        """Bild-Analyse via VLM (z-ai vision)."""
        if not ZAI_AVAILABLE:
            return {"error": "Module not available: core.zai_integration is not installed"}
        try:
            zai = get_zai()
            result = zai.vision(prompt=prompt, image=image)
            if isinstance(result, dict):
                return {"prompt": prompt, "image": image, "result": str(result)[:5000]}
            return {"prompt": prompt, "image": image, "result": str(result)[:5000]}
        except Exception as e:
            return {"error": f"ai_vision fehlgeschlagen: {str(e)}"}

    @staticmethod
    def _tool_ai_video(prompt: str, duration: int = 5, quality: str = "standard") -> dict:
        """AI-Video generieren via z-ai."""
        if not ZAI_AVAILABLE:
            return {"error": "Module not available: core.zai_integration is not installed"}
        try:
            zai = get_zai()
            result = zai.generate_video(prompt=prompt, duration=duration, quality=quality)
            if isinstance(result, dict):
                return {"prompt": prompt, "duration": duration, "quality": quality, "result": str(result)[:5000]}
            return {"prompt": prompt, "duration": duration, "quality": quality, "result": str(result)[:5000]}
        except Exception as e:
            return {"error": f"ai_video fehlgeschlagen: {str(e)}"}

    @staticmethod
    def _tool_ai_tts(text: str, voice: str = "alloy", speed: float = 1.0, format: str = "mp3") -> dict:
        """Text-to-Speech via z-ai."""
        if not ZAI_AVAILABLE:
            return {"error": "Module not available: core.zai_integration is not installed"}
        try:
            zai = get_zai()
            result = zai.text_to_speech(text=text, voice=voice, speed=speed, format=format)
            if isinstance(result, dict):
                return {"text": text[:200], "voice": voice, "speed": speed, "format": format, "result": str(result)[:5000]}
            return {"text": text[:200], "voice": voice, "speed": speed, "format": format, "result": str(result)[:5000]}
        except Exception as e:
            return {"error": f"ai_tts fehlgeschlagen: {str(e)}"}

    @staticmethod
    def _tool_ai_asr(file: str = "", base64: str = "") -> dict:
        """Speech-to-Text via z-ai."""
        if not ZAI_AVAILABLE:
            return {"error": "Module not available: core.zai_integration is not installed"}
        try:
            zai = get_zai()
            result = zai.speech_to_text(file=file, base64=base64)
            if isinstance(result, dict):
                return {"file": file, "result": str(result)[:5000]}
            return {"file": file, "result": str(result)[:5000]}
        except Exception as e:
            return {"error": f"ai_asr fehlgeschlagen: {str(e)}"}

    # ═══════════════════════════════════════════════════
    # HERMES-INSPIRED TOOL IMPLEMENTATIONS (5)
    # ═══════════════════════════════════════════════════

    @staticmethod
    def _tool_rag_ingest(source: str, type: str = "text") -> dict:
        """Dokument in RAG-Wissensdatenbank aufnehmen."""
        if not RAG_AVAILABLE:
            return {"error": "Module not available: core.rag is not installed"}
        try:
            valid_types = ["file", "url", "text"]
            if type not in valid_types:
                return {"error": f"Ungültiger Typ '{type}'. Gültig: {valid_types}"}
            rag = RAGSystem()
            result = rag.ingest(source=source, source_type=type)
            return {"source": source, "type": type, "result": str(result)[:5000]}
        except Exception as e:
            return {"error": f"rag_ingest fehlgeschlagen: {str(e)}"}

    @staticmethod
    def _tool_rag_search(query: str, top_k: int = 5) -> dict:
        """RAG-Wissensdatenbank durchsuchen."""
        if not RAG_AVAILABLE:
            return {"error": "Module not available: core.rag is not installed"}
        try:
            rag = RAGSystem()
            result = rag.search(query=query, top_k=top_k)
            return {"query": query, "top_k": top_k, "result": str(result)[:5000]}
        except Exception as e:
            return {"error": f"rag_search fehlgeschlagen: {str(e)}"}

    @staticmethod
    def _tool_self_reflect(task: str, result: str) -> dict:
        """Eigene Ausgabequalität reflektieren."""
        if not REFLECTION_AVAILABLE:
            return {"error": "Module not available: core.reflection is not installed"}
        try:
            engine = ReflectionEngine()
            reflection = engine.reflect(task=task, result=result)
            return {"task": task[:200], "reflection": str(reflection)[:5000]}
        except Exception as e:
            return {"error": f"self_reflect fehlgeschlagen: {str(e)}"}

    @staticmethod
    def _tool_plan_task(task: str, agents: str = "", tools: str = "") -> dict:
        """Schrittweisen Plan erstellen."""
        if not REFLECTION_AVAILABLE:
            return {"error": "Module not available: core.reflection is not installed"}
        try:
            engine = PlanningEngine()
            plan = engine.plan(task=task, agents=agents, tools=tools)
            return {"task": task[:200], "agents": agents, "tools": tools, "plan": str(plan)[:5000]}
        except Exception as e:
            return {"error": f"plan_task fehlgeschlagen: {str(e)}"}

    @staticmethod
    def _tool_self_correct(task: str, code: str, error: str) -> dict:
        """Code mit Fehlerinfo selbst korrigieren."""
        if not REFLECTION_AVAILABLE:
            return {"error": "Module not available: core.reflection is not installed"}
        try:
            engine = ReflectionEngine()
            correction = engine.self_correct(task=task, code=code, error=error)
            return {"task": task[:200], "correction": str(correction)[:5000]}
        except Exception as e:
            return {"error": f"self_correct fehlgeschlagen: {str(e)}"}
