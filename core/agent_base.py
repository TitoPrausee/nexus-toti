...
class AgentBase:
    ...
    def quick_response(self, message: str) -> str:
        ...
        self._conversation.append(Message(role="assistant", content=content))
        if len(self._conversation) > 40:
            self._conversation = self._conversation[-40:]

        self.memory.session_log("user", message, "USER")
        self.memory.session_log("assistant", content, self.AGENT_ID)
