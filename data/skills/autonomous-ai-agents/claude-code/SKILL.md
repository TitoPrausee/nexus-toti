...
## Lessons from CD Shelf Player Agent Run ($4.06, 31 turns)

- **30 turns ≈ 1 scaffold task** — not enough for code + tests + git push. Budget 5-10 extra turns for git operations, or push manually after agent finishes.
- **`error_max_turns` is NOT a failure** — agent likely completed most work. Check `git diff` and files before retrying.
- **GitLab SSH fails on fresh servers** — always do `ssh-keyscan gitlab.com >> ~/.ssh/known_hosts` OR use HTTPS with PAT in URL.
- **Rebase conflicts** — when agent commits on a fresh scaffold, `git pull --rebase` on the existing remote README causes conflicts. Use `git checkout --theirs` strategy.
- **Shell escaping** — write prompts to `/tmp/agent-prompts/task.md` and use `$(cat /tmp/agent-prompts/task.md)` instead of inline strings.
- **Agent doesn't know your git credentials** — include `git -c user.name="..." -c user.email="..."` in the prompt, or set globals before launching.
...
## Parallel Background Agent Pattern (Hermes Orchestration)

### Pattern: Multiple agents working on the same repo simultaneously

Use `terminal(background=true)` to launch multiple Claude Code agents in parallel on different tasks in the same repository. This is ideal for independent features or a fix-agent following implementation agents.

```python
# 1. Write prompt files first (avoids shell escaping issues with complex prompts)
write_file(path="/tmp/agent-prompts/task-memory.md", content="...")
write_file(path="/tmp/agent-prompts/task-rebrand.md", content="...")

# 2. Launch agents in background with notification
terminal(background=True, notify_on_complete=True,
         command='export ANTHROPIC_API_KEY=... ANTHROPIC_BASE_URL=... && '
                 'cd /project && claude -p "$(cat /tmp/agent-prompts/task-memory.md)" '
                 '--dangerously-skip-permissions --max-turns 50 --output-format json '
                 '2>&1 | tee /tmp/agent-memory.log',
         watch_patterns=["commit","pass","ERROR"],
         timeout=600)

# 3. Monitor progress
process(action="poll", session_id="proc_xxx")

# 4. After completion, review and commit changes
terminal(command="cd /project && git diff --stat")
terminal(command="cd /project && bun test")  # verify tests pass
terminal(command="cd /project && git add -A && git commit -m '...' && git push")
```

### Critical Lessons

1. **50-turn limit is real** — agents hit `error_max_turns` before finishing large tasks. Budget 50 turns ≈ 1 major feature with tests. For bigger tasks, split into sequential agents using `--resume`.

2. **Conflicting changes** — parallel agents modifying the same files will create merge conflicts. Assign clear file ownership per agent or run them sequentially for overlapping areas.

3. **Shell escaping** — complex prompts with quotes/special chars break in `tmux send-keys`. ALWAYS write prompts to file and use `"$(cat /tmp/prompt.md)"` instead.

4. **Layer composition bugs** — Effect-TS agents often miss required Layer dependencies in tests. When an agent writes new Effect-TS services, ALWAYS verify `bun test` passes before committing. A "fix agent" run afterwards is usually needed.

5. **Module resolution after rebrand** — renaming packages (e.g., `@opencode-fusion` → `@toti`) requires `bun install` to update symlinks before tests pass. Run `bun install` immediately after package.json name changes.

6. **Fix agent pattern** — after implementation agents, launch a fix agent with lower turn count (15-30) and a targeted prompt to resolve TS errors and test failures. This is more efficient than trying to get implementation agents to produce perfect code.

7. **Agent cost awareness** — a 50-turn agent run costing $7-11 USD is normal for complex tasks. Track `total_cost_usd` in the JSON output. Budget accordingly.

8. **`bun` PATH issue** — background processes may not have `~/.bun/bin` on PATH. Always `export PATH="$HOME/.bun/bin:$HOME/.local/bin:$PATH"` in the command or it won't find `bun test`.

9. **Git-as-memory pattern** — when Hermes agent memory is limited (2,200 chars), store persistent project context in a file like `AGENT_CONTEXT.md` in the Git repo. This survives server restarts and session resets. On session resume, read this file first to restore full context.

10. **Incremental verification after agents** — after agents finish, always: (a) `bun install` to resolve package changes, (b) `bun test` to verify test suite, (c) `git diff --stat` to review scope, (d) commit then push. Agents frequently leave TS type errors that only surface at test time.
11. **Agent delegation priority over manual coding** — When the user says "work on the agent system", prefer delegating to Claude Code agents rather than writing code yourself. The goal is to prove the agent infrastructure works by using it. Write prompt files to /tmp/agent-prompts/, launch with print mode, monitor, verify, push manually.
12. **svelte-check for Tauri/Svelte projects** — Don't try `vite build` or `vite dev` in foreground (they start long-lived servers). Use `npx svelte-check --tsconfig ./tsconfig.json` instead — it's fast, returns 0 errors if components compile, no server needed.
13. **GitLab push after agent** — Agents often can't push (SSH host key, PAT expired). Plan to push manually after agent completes. Use HTTPS with PAT as fallback: `git remote set-url origin https://USER:PAT@gitlab.com/USER/REPO.git`.

### Prompt Template for Background Agents

```markdown
You are working on [PROJECT_NAME], a [TECH_STACK] project. Read CLAUDE.md first for context.

Your task: [SPECIFIC_ISSUE_AND_WHAT_TO_DO]

## Key Files
- [list relevant source files and paths]
- [mention any Effect-TS patterns or project conventions]
